...
Andrej Karpathy 가 제안한 [LLM Wiki 패턴](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 은 이 문제의 절반을 풉니다 — LLM 이 원본 소스를 *한 번* 읽고, 결과를 위키로 누적하면 시간이 지날수록 위키 자체가 1차 소스가 됩니다. RAG 와 달리 매번 처음부터 다시 합성하지 않아도 됩니다.
...
Cursor · Codex · Gemini CLI 에서도 별도 설정 없이 그대로 작동합니다.

자동 컨텍스트 로드가 없는 환경 (일반 웹 챗 등) 에선 매 세션 시작 시 `CLAUDE.md` 내용을 시스템 프롬프트에 직접 붙여 넣으셔야 합니다.

`tools/instill_sched.py` 는 **Python 3.10+** 필요합니다.
...
2. agent 에게 경로를 주고 "이거 ingest 해줘" 라고 합니다. agent 가 알아서 위키로 정리하고 학습 준비까지 마쳐줍니다.
...
- 원본 영감: [Karpathy 의 LLM Wiki gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)
