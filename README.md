...
스케줄링 알고리즘은 **FSRS-4.5** 를 씁니다. Anki 가 2024 부터 기본으로 채택한 최신 알고리즘이에요. 직접 LLM 이 계산하면 오차가 생길 수 있어서 `tools/instill_sched.py` 라는 작은 스크립트로 분리했습니다. Python 표준 라이브러리만 쓰니까 별도 설치는 필요 없습니다.
